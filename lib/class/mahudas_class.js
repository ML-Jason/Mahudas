...
const coreLogger = require('../logger/coreLogger');
...
class Mahudas extends Koa {
  ...
  async loadApp() {
    ...
    const { Logger, Transports, Format } = appLogger(this);
    this.logger = Logger;
    ...
    // 遍尋schedule，如果有要立即執行的schedule，就執行它
    const deepRun = (obj) => {
      if (obj.constructor.name === 'ScheduleJob') {
        if (obj.immediate) obj.run();
      } else {
        Object.entries(obj).forEach(([, instance]) => {
          deepRun(instance);
        });
      }
    };
    deepRun(this.schedule);
  }
  ...
  async close() {
    // 停止所有的schedule
    // 因為this.schedule可能是巢狀結構，因此需要一層一層進去stop
    const deepStop = (obj) => {
      if (obj.constructor.name === 'ScheduleJob') {
        obj.stop();
      } else {
