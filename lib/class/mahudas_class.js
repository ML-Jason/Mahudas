...
class Mahudas extends Koa {
  constructor(options = {}) {
    ...
    this.logger = logger.mountConsoleLogger;

    this.logger.info(`\x1B[36m▉ NODE_ENV=${process.env.NODE_ENV}\x1B[0m`);
    this.logger.info(`\x1B[36m▉ APP_ENV=${process.env.APP_ENV}\x1B[0m`);
  ...
  async loadApp() {
    ...
    appContainer.loadFileLogger(logger.getFileTransports(this));
  ...
  startServer() {
    ...
    if (this.appInfo.startMode !== 'mount') {
      ...
      this.logger.info(`\x1B[36m▉ serverDidReady, running on http://localhost:${port}\x1B[0m`);
    }
    ...
    this.logger.info('\x1B[36m▉ Memory usage:\x1B[0m', memoryInfo);
