...
const configLoader = require('../loader/config_loader');
const pluginConfigLoader = require('../loader/plugin_config_loader');
const serviceLoder = require('../loader/service_loader');
const routerLoader = require('../loader/router_loader');
const controllerLoader = require('../loader/controller_loader');
const middlewareLoader = require('../loader/middleware_loader');
const extendLoader = require('../loader/extend_loader');
...
class AppContainer {
  ...
  constructor(app, rootPath) {
    ...
    if (rootPath === app.appInfo.root) {
      this.isMainApp = true;
      if (app.appInfo.startMode !== 'standalone') {
        const mahudas = new AppContainer(app, path.join(__dirname, '../..'));
  ...
  loadConfig() {
    ...
    configs = extend(true, configs, configLoader(this.rootPath));
    if (this.isMainApp) {
  ...
  findPlugins() {
    const pluginConfig = pluginConfigLoader(this.rootPath);
    Object.entries(pluginConfig).forEach(([key, v]) => {
      if (v.enable === true) {
        ...
        if (fs.existsSync(pluginPath)) {
          console.log(`\x1B[36m▉ Loading plugin:${key}\x1B[0m`);
          const plugin = new AppContainer(this.app, pluginPath);
  ...
  loadService() {
    ...
    const myservices = serviceLoder(this.app, this.rootPath);
    ...
    if (this.isMainApp) {
      ...
      this.app.service = new BaseContext({ app: this.app, subModules: serviceMap });
      ...
      Object.defineProperty(this.app.context, 'service', {
        get() {
          ...
          const instance = new BaseContext({ ctx: this, subModules: serviceMap });
  ...
  loadExtend() {
    ...
    extendLoader(this.app, this.rootPath);
  }
  ...
  loadMiddleware() {
    ...
    middlewareLoader(this.app, this.rootPath);
  }

  // controller及router只有在主應用才會生效
  // 因此不命令subApps去載入
  loadRouter() {
    controllerLoader(this.app, this.rootPath);
    routerLoader(this.app, this.rootPath);
  }
}
