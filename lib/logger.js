      ...
      if (config.outputFile) {
        transport.push(
          new transports.DailyRotateFile({
            filename: path.join(app.appInfo.root, config.logDir, `${process.env.APP_ENV}_combined-%DATE%.log`),
            format: format.combine(
              format.timestamp(),
              format.splat(),
              excludeFormat,
        ...
        transport.push(
          new transports.DailyRotateFile({
            filename: path.join(app.appInfo.root, config.logDir, `${process.env.APP_ENV}_error-%DATE%.log`),
            level: 'error',
            format: format.combine(
              format.timestamp(),
              format.splat(),
